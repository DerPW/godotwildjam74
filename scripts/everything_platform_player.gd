...
@export var air_jump_has_ability := false
...
@export var dash_directions := 8 # 2, 4, 8
var dash_duration := .2
@export var	dash_has_ability := true
...
func _physics_process(delta):
	if process_self:
		...
		if can_move() and (on_floor() or (wall_slide_latched != 0 and dash_refresh_timer_update_on_wall_latch)):
			dash_refresh_timer_update = true
		if can_move() and (on_floor() or (wall_slide_latched != 0 and air_jump_refresh_on_wall_latch)):
...
func current_gravity():
	...
	if jumping == true and Input.is_action_pressed("move_up") and last_solid == Vector2.DOWN and velocity.y < 0 and abs(velocity.y) < peak_gravity_threshold:
		return peak_gravity
	else:
		return gravity
...
func jump(_delta):
	...
	if Input.is_action_just_released("move_up") and jumping:#(jump_buffer_timer > 0 or jumping):
